...
document.addEventListener("DOMContentLoaded", function () {
  ...
  images.forEach((img) => {
    img.addEventListener("mouseover", function () {
      setImage(img, imgContainer);
    });

    img.addEventListener("mouseout", function () {
      removeImage(img, imgContainer);
    });
  });
});
...
function setImage(image, imgContainer) {
  imgContainer.src = image.src;
  image.style.border = "10px solid green";
}

function removeImage(image, imgContainer) {
  imgContainer.src = containerImgSrc;
